...
class UNetSpherical(UNet, torch.nn.Module):
    ...
    def __init__(self,
        ...
        if pool_method in ('interp', 'maxval', 'maxarea', 'learn'):
            ...
            self.pool2, self.unpool2 = PoolUnpoolBlock.getGeneralPoolUnpoolLayer(src_graph=self.graphs[1],
                                                                                 ...
                                                                                 pool_method=pool_method)
            self.pool3, self.unpool3 = PoolUnpoolBlock.getGeneralPoolUnpoolLayer(src_graph=self.graphs[2],
                                                                                 dst_graph=self.graphs[3], 
                                                                                 pool_method=pool_method)
            ...
            self.pool2, self.unpool2 = PoolUnpoolBlock.getPoolUnpoolLayer(sampling=sampling, 
                                                                          ...
                                                                          kernel_size=kernel_size_pooling, 
                                                                          lonlat_ratio=lonlat_ratio)
            self.pool3, self.unpool3 = PoolUnpoolBlock.getPoolUnpoolLayer(sampling=sampling, 
                                                                          pool_method=pool_method, 
                                                                          kernel_size=kernel_size_pooling, 
                                                                          lonlat_ratio=lonlat_ratio)
        else:
        ...
        # Encoding block 3
        self.conv3 = ResBlock(128 * 2, (192 * 2, 256 * 2),
                              laplacian = self.laplacians[2],
                              convblock_kwargs = convblock_kwargs)
        
        # Encoding block 4
        self.conv4 = ResBlock(256 * 2, (512 * 2, 256 * 2),
                              laplacian = self.laplacians[3],
                              convblock_kwargs = convblock_kwargs)
        
        ##--------------------------------------------------------------------.
        ### Decoding blocks 
        # Decoding level 3
        self.uconv3 = ResBlock(512 * 2, (256 * 2, 128 * 2),
                      laplacian = self.laplacians[2],
                      convblock_kwargs = convblock_kwargs)
    ...
    def encode(self, x):
        ...
        # Level 4
        x_enc4_ini, idx3 = self.pool3(x_enc3)
        x_enc4 = self.conv4(x_enc4_ini)
        
        return x_enc4,x_enc3, x_enc2, x_enc1, idx3, idx2, idx1, x_last_timestep  
    
    ##------------------------------------------------------------------------.
    def decode(self, x_enc4,x_enc3, x_enc2, x_enc1, idx3, idx2, idx1, x_last_timestep ): 
        """Define UNet decoder."""
        ## Define decoder computation
        # - Level 3
        x = self.unpool3(x_enc4, idx3)
...
# class SphericalUNetTemporalLSTM(UNetSpherical):
...
#         self.lstm_l0 = nn.LSTM(input_size=self.input_n_node *  self.input_n_feature, hidden_size=self.input_n_node *  self.input_n_feature, batch_first=True)
...
#         device = x.device
#         encoders_l0 = []
#         for idx in range(self.input_n_time):
